...
async def get_dataset_schema(dataset_id: int) -> str:
    ...
    Returns:
        Markdown formatted schema with tables, columns, types, and AI descriptions
    """
    start_time = time.time()
    db = next(get_db())
    ...
    try:
        ...
        dataset = db.query(Dataset).filter(Dataset.id == dataset_id).first()
        if not dataset or not dataset.is_active:
            return formatter.format_error('Dataset not found or inactive', f'Dataset ID: {dataset_id}')
        ...
        # Format as Markdown
        return formatter.format_dataset_schema(
            dataset_id=dataset_id,
